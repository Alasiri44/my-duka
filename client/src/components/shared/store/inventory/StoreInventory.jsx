...
const StoreInventory = () => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (sortByStock === "asc") {
      result = result.sort((a, b) => (a.quantity_on_hand || 0) - (b.quantity_on_hand || 0));
    } else if (sortByStock === "desc") {
      result = result.sort((a, b) => (b.quantity_on_hand || 0) - (a.quantity_on_hand || 0));
    }
      ...
      {showSummary && (
        <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          {Object.entries(categoryTotals).map(([cat, val]) => (
            <div key={cat} className="bg-white border border-[#d7d0c8] p-4 rounded shadow-sm">
